...
public class BehaviorTree extends PApplet {
	...
	public void draw() {
		...
		drawObstacles();
		if((vxpos>=2 * width / 10 && vypos>= 1 * height / 7 && vxpos< 4 * width / 10 && vypos< 2 * height / 7) && vre==false){
		...
		if((xpos>=6 * width / 10 && ypos>= 1 * height / 7 && xpos< 8 * width / 10 && ypos< 2 * height / 7 )||infected == true){
			//suicide
			//seek enemy
			infected = true;
			...
			karrive.draw();
			if ((float) wander.character.getx() >(4.5f * width / 10)&& (float) wander.character.getx() <= (5.5f * width / 10)
					&& ((float) wander.character.gety() <= (3.75f * height / 7) && (float) wander.character.gety() >(3f * height / 7))) {
				vwander.draw();
				area = "terrain";
				type = "enemy";
				mode = "wander";
	...
	public void generatelog() {
		...
		*/newRow.setFloat("dist",
				dist((float) vwander.character.getx(), (float) vwander.character.gety(), xpos, ypos));
		...
		saveTable(table, "data/LogFile.csv");
	}
	...
	public static void main(String[] args) {
		// TODO Auto-generated method stub
		PApplet.main(new String[] { "--present", "assignment.BehaviorTree" });
	}
